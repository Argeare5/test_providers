...
import { getNameByChainId } from '../../utils/getNameByChainId';
...
export function ConnectWalletButton({ onClick }: ConnectWalletButtonProps) {
  ...
  const wrongNetwork = activeWallet?.wrongNetwork;
            ...
            <Flex css={{ alignItems: 'center', justifyContent: 'center' }}>
              <Flex
                ...
                <Typography variant="buttonSmall">
                  {getNameByChainId(activeWallet?.chainId || 1)}
                </Typography>
              </Flex>

              <Button
                onClick={onClick}
                error={wrongNetwork || lastTransactionError}
                success={lastTransactionSuccess}
                color={
                  wrongNetwork || lastTransactionSuccess || lastTransactionError
                    ? 'dark'
                    : 'gray'
                }
                leftComponent={
